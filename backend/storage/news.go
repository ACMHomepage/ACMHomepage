...
func (db DB) CreatetNews(ctx context.Context, news *News) error {
...
func (db DB) GetNews(ctx context.Context, news *News, id string) error {
	err := db.NewSelect().Model(news).Where("id = ?", id).Scan(ctx)
	return err
}

func (db DB) UpdateNews(ctx context.Context, news *News, id string) error {
	_, err := db.NewUpdate().Model(news).Where("id = ?", id).Exec(ctx)
	return err
}

func (db DB) DeleteNews(ctx context.Context, news *News, id string) error {
